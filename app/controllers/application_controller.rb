class ApplicationController < ActionController::Base
  ...
  def authorize_menu_access_globally
    ...
    puts("permisos: #{PermissionRegistry.requires_menu_check?(current_controller, current_action)}")
    return unless PermissionRegistry.requires_menu_check?(current_controller, current_action)
    ...
    if required_menu.present?
      puts("Permisos: #{Current.user.can_access_menu?(required_menu)}")
